...
for (i in SYSTEMS) {
  ...
  pdf_name <- paste(as.character(i),"_",as.Date(as.POSIXlt(enddate, origin = "1970-1-1", format = "%Y-%m-%d")),"_BOXP_MEM_",DAYS,"d.pdf", sep = "")
  ...
  #################################################################################
  newPage()
  plot_title<- paste("Memory Utilization Boxplot by Day\nHost:", as.character(i), StartDate," to ", EndDate)
