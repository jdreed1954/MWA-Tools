...
genplot_cor <- function(system) {
  ...
  pdf_name <- paste(as.character(i),"_",as.Date(as.POSIXlt(enddate, 
      origin = "1970-1-1", format = "%Y-%m-%d")),"_BOXP_CPUAct_",DAYS,"d.pdf", sep = "")
  ...
  plot_title<- paste("Number of Cores Active Boxplot by Day\nHost:", 
                     as.character(i), StartDate," to ", EndDate)
...
genplot_cpu <- function(system){
  ...
  # Create the name of the plot file in the pdf command below
  pdf_name <- paste(as.character(i),"_",as.Date(as.POSIXlt(enddate, 
      origin = "1970-1-1", format = "%Y-%m-%d")),"_BOXP_CPU_",DAYS,"d.pdf", sep = "")
