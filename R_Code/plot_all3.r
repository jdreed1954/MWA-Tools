...
for (i in SYSTEMS) {
  ...
  #                           C P U                                               #
  #################################################################################
  xts <- zoo(CPU.User, order.by = x$dt)
  plot(xts,main="CPU User %",ylab="%", xlab="Date")
  grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
	hist(CPU.User,breaks=50,main="CPU User %")
  putLegend(xts)
  
  pnum <- pnum +1
  printHeaderFooter(i, pnum, startdate, enddate)

  xts <- zoo(CPU.Sys,order.by=x$dt)
  plot(xts,main="CPU Sys %",ylab="%", xlab="Date")
  grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
	hist(CPU.Sys,breaks=50,main="CPU Sys %")
  putLegend(xts)
   
  pnum <- pnum +1
  printHeaderFooter(i, pnum, startdate, enddate)
     
  xts <- zoo(CPU.Intrpt, order.by=x$dt)
  plot(xts,main="CPU Interrupt %",ylab="%", xlab="Date")
  grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
  hist(CPU.Intrpt,breaks=50,main="CPU Interrupt %")
  putLegend(xts)
   
  pnum <- pnum +1
  printHeaderFooter(i, pnum, startdate, enddate)
  
  xts <- zoo(CPU.SysCall, order.by=x$dt)
  plot(xts,main="CPU Syscalll %",ylab="%", ylim=c(0,110), xlab="Date", col="red")
  grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
  hist(CPU.SysCall,breaks=50,main="CPU Syscall %", col="red")
  putLegend(xts)
  ...
  printHeaderFooter(i, pnum, startdate, enddate)   
   ...
   plot(xts,main="Memory Utilization Percent",ylab="Mem.Pct", xlab="Date")
   grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
   ...
   plot(xts,main="Inbound Packet Rate",ylab="InPkt.Rt", xlab="Date")
   grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
   ...
   xts <- zoo(OutPkt.Rt, order.by=x$dt)
   plot(xts,main="Outbound Packet Rate",ylab="OutPkt.Rt", xlab="Date")
   grid(nx=31, ny=10, col = "lightgray", lty = "dotted")
   hist(OutPkt.Rt,breaks=50,main="Outbound Packet Rate")
   putLegend(xts)
